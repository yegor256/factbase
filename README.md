...
import 410860 bytes (20000 facts)                                  0.04
insert 10 facts                                                    0.04
query 10 times w/txn                                               2.52
query 10 times w/o txn                                             0.05
modify 10 attrs w/txn                                              1.92
delete 10 facts w/txn                                              0.96
...
query 5000 facts building index on-the-fly                         0.13
(and (eq what 'issue-was-closed') (exists... -> 200                1.23
(and (eq what 'issue-was-closed') (exists... -> 200/txn            1.32
(and (eq what 'issue-was-closed') (exists... -> zero               1.26
(and (eq what 'issue-was-closed') (exists... -> zero/txn           1.33
transaction rollback on factbase with 100000 facts                 0.23
(gt time '2024-03-23T03:21:43Z')                                   0.33
(gt cost 50)                                                       0.21
(eq title 'Object Thinking 5000')                                  0.04
(and (eq foo 42.998) (or (gt bar 200) (absent z...                 0.05
(and (exists foo) (not (exists blue)))                             1.73
(eq id (agg (always) (max id)))                                    2.85
(join "c<=cost,b<=bar" (eq id (agg (always) (ma...                 4.12
(and (eq what "foo") (join "w<=what" (and (eq i...                 7.79
delete!                                                            0.57
(and (eq issue *) (eq repository *) (eq what '*') (eq where '*'))  2.75
Taped.append() x50000                                              0.15
Taped.each() x125                                                  1.62
Taped.delete_if() x375                                             0.84
50000 facts: read-only txn (no copy needed)                       15.84
50000 facts: rollback txn (no copy needed)                        15.52
50000 facts: insert in txn (copy triggered)                        6.77
50000 facts: modify in txn (copy triggered)                       47.29
100000 facts: read-only txn (no copy needed)                      28.58
100000 facts: rollback txn (no copy needed)                       27.39
100000 facts: insert in txn (copy triggered)                      13.48
100000 facts: modify in txn (copy triggered)                      85.58
```

The results were calculated in [this GHA job][benchmark-gha]
on 2025-12-06 at 10:34,
on Linux with 4 CPUs.
<!-- benchmark_end -->

[benchmark-gha]: https://github.com/yegor256/factbase/actions/runs/19987207481
