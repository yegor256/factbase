...
insert 20000 facts                                                 0.65
export 20000 facts                                                 0.02
import 411061 bytes (20000 facts)                                  0.02
insert 10 facts                                                    0.04
query 10 times w/txn                                               2.42
query 10 times w/o txn                                             0.12
modify 10 attrs w/txn                                              2.27
delete 10 facts w/txn                                              2.83
...
(and (eq what 'issue-was-closed') (exists... -> 200                1.15
(and (eq what 'issue-was-closed') (exists... -> 200/txn            1.22
(and (eq what 'issue-was-closed') (exists... -> zero               1.17
(and (eq what 'issue-was-closed') (exists... -> zero/txn           1.29
transaction rollback on factbase with 100000 facts                 0.22
(gt time '2024-03-23T03:21:43Z')                                   0.46
(gt cost 50)                                                       0.20
(eq title 'Object Thinking 5000')                                  0.03
(and (eq foo 42.998) (or (gt bar 200) (absent z...                 0.03
(and (exists foo) (not (exists blue)))                             1.71
(eq id (agg (always) (max id)))                                    3.01
(join "c<=cost,b<=bar" (eq id (agg (always) (ma...                 4.56
(and (eq what "foo") (join "w<=what" (and (eq i...                 7.28
delete!                                                            0.52
(and (eq issue *) (eq repository *) (eq what '*') (eq where '*'))  0.28
Taped.append() x50000                                              0.08
Taped.each() x125                                                  1.65
Taped.delete_if() x375                                             0.87
50000 facts: read-only txn (no copy needed)                        8.22
50000 facts: rollback txn (no copy needed)                         7.98
50000 facts: insert in txn (copy triggered)                        4.18
50000 facts: modify in txn (copy triggered)                       38.64
100000 facts: read-only txn (no copy needed)                      16.37
100000 facts: rollback txn (no copy needed)                       16.53
100000 facts: insert in txn (copy triggered)                       8.30
100000 facts: modify in txn (copy triggered)                      74.22
```

The results were calculated in [this GHA job][benchmark-gha]
on 2026-01-28 at 03:30,
on Linux with 4 CPUs.
<!-- benchmark_end -->

[benchmark-gha]: https://github.com/yegor256/factbase/actions/runs/21423740177
