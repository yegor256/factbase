...
insert 20000 facts                                                 0.60
export 20000 facts                                                 0.02
import 410945 bytes (20000 facts)                                  0.02
insert 10 facts                                                    0.03
query 10 times w/txn                                               2.38
query 10 times w/o txn                                             0.12
modify 10 attrs w/txn                                              2.19
delete 10 facts w/txn                                              2.55
build index on 5000 facts                                          0.05
export 5000 facts with index                                       0.03
import 5000 facts with persisted index                             0.05
query 5000 facts using persisted index                             0.13
...
(and (eq what 'issue-was-closed') (exists... -> 200/txn            1.14
(and (eq what 'issue-was-closed') (exists... -> zero               1.08
(and (eq what 'issue-was-closed') (exists... -> zero/txn           1.17
transaction rollback on factbase with 100000 facts                 0.22
(gt time '2024-03-23T03:21:43Z')                                   0.36
(gt cost 50)                                                       0.17
(eq title 'Object Thinking 5000')                                  0.03
(and (eq foo 42.998) (or (gt bar 200) (absent z...                 0.06
(and (exists foo) (not (exists blue)))                             1.63
(eq id (agg (always) (max id)))                                    2.80
(join "c<=cost,b<=bar" (eq id (agg (always) (ma...                 4.24
(and (eq what "foo") (join "w<=what" (and (eq i...                 6.84
delete!                                                            0.51
(and (eq issue *) (eq repository *) (eq what '*') (eq where '*'))  0.28
Taped.append() x50000                                              0.07
Taped.each() x125                                                  1.59
Taped.delete_if() x375                                             0.82
50000 facts: read-only txn (no copy needed)                        8.07
50000 facts: rollback txn (no copy needed)                         7.85
50000 facts: insert in txn (copy triggered)                        4.18
50000 facts: modify in txn (copy triggered)                       37.48
100000 facts: read-only txn (no copy needed)                      16.93
100000 facts: rollback txn (no copy needed)                       17.12
100000 facts: insert in txn (copy triggered)                       8.94
100000 facts: modify in txn (copy triggered)                      71.86
```

The results were calculated in [this GHA job][benchmark-gha]
on 2026-01-01 at 09:43,
on Linux with 4 CPUs.
<!-- benchmark_end -->

[benchmark-gha]: https://github.com/yegor256/factbase/actions/runs/20636378912
