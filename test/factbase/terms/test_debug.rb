...
class TestDebug < Factbase::Test
  ...
  def test_assert_raises_when_second_arg_not_term
    e = assert_raises(StandardError) do
      Factbase::Term.new(:assert, ['message', 'not_a_term']).evaluate(fact, [], Factbase.new)
    end
  ...
  def test_assert_raises_when_too_many_args
    e = assert_raises(StandardError) do
      Factbase::Term.new(:assert, ['message', Factbase::Term.new(:gt, [:foo, 0]), 'extra']).evaluate(fact, [], Factbase.new)
    end
