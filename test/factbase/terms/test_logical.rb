...
class TestLogical < Factbase::Test
  def test_not_matching
    refute(Factbase::Term.new(:not, [Factbase::Term.new(:always, [])]).evaluate(fact('foo' => [100]), [], Factbase.new))
  end
  ...
  def test_either
    assert_equal(
      [42],
      Factbase::Term.new(:either, [Factbase::Term.new(:at, [5, :foo]), 42]).evaluate(
        fact('foo' => 4), [],
        Factbase.new
      )
    )
  ...
  def test_or_matching
    t = Factbase::Term.new(:or, [Factbase::Term.new(:eq, [:foo, 4]), Factbase::Term.new(:eq, [:bar, 5])])
  ...
  def test_when_matching
    t = Factbase::Term.new(:when, [Factbase::Term.new(:eq, [:foo, 4]), Factbase::Term.new(:eq, [:bar, 5])])
