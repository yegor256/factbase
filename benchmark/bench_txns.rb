...
def bench_txns(bmk, _fb, cycles)
  ...
  totals.each do |total|
    ...
    raise(StandardError, 'not enough facts') unless fb_read_plain.query('(always)').each.any?
    ...
    raise(StandardError, 'not enough facts') unless fb_read_txn.query('(always)').each.any?
    ...
    raise(StandardError, 'not enough facts') unless fb_ins_plain.query('(always)').each.any?
    ...
    raise(StandardError, 'not enough facts') unless fb_ins_txn.query('(always)').each.any?
    ...
    raise(StandardError, 'not enough facts') if fb_mod_plain.query('(eq foo 0)').each.to_a.size != cycles
    ...
    raise(StandardError, 'not enough facts') if fb_mod_txn.query('(eq foo 0)').each.to_a.size != cycles
